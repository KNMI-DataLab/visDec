...
results <- apply(daylightImages, 1, function(x) {

  im <- subim(load.image(x["filePath"]), y > 16) 
  imT <- RGBtoHSV(im)
  data.table(name = x["filePath"],
             dateTime = x["dateTime"],
...
imageSummary <- rbindlist(results)
imageSummary[, dateTime:= as.POSIXct(dateTime)]
```
