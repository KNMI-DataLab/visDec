...
detect.edges(load.image(filenames[1])) %>% sqrt %>% plot
...
imageSummary <- foreach(file = iter(filenames), .combine = rbind) %dopar% {
  ...
  data.table(name = fileInformation$name,
             ...
             meanEdge = detect.edges(im, 3) %>% threshold(5) %>% sqrt %>% mean
  )
}
```
...
ggplot(imageSummary, aes(x = meanEdge, y = FS261, col = visibility)) + geom_point()
...
ggplot(imageSummary[hour %in% seq(7, 16, by = 1), ], aes(x = log(FS261), y = meanEdge, col = visibility)) + geom_point()
